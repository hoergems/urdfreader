...
bool construct_model (RigidBodyDynamics::Model* rbdl_model, 
	...
	RigidBodyDynamics::Math::Vector3d root_inertial_rpy;
	RigidBodyDynamics::Math::Vector3d root_inertial_position;
	RigidBodyDynamics::Math::Matrix3d root_inertial_inertia;
	...
	if (root->inertial) {
		...
		RigidBodyDynamics::Body root_link = RigidBodyDynamics::Body (root_inertial_mass,
	...
	for (j = 0; j < joint_names.size(); j++) {
		...
		// create the joint
		RigidBodyDynamics::Joint rbdl_joint;
		if (urdf_joint->type == urdf::Joint::REVOLUTE || urdf_joint->type == urdf::Joint::CONTINUOUS) {
			rbdl_joint = RigidBodyDynamics::Joint (RigidBodyDynamics::Math::SpatialVector (urdf_joint->axis.x, urdf_joint->axis.y, urdf_joint->axis.z, 0., 0., 0.));
		} else if (urdf_joint->type == urdf::Joint::PRISMATIC) {
			rbdl_joint = RigidBodyDynamics::Joint (RigidBodyDynamics::Math::SpatialVector (0., 0., 0., urdf_joint->axis.x, urdf_joint->axis.y, urdf_joint->axis.z));
		} else if (urdf_joint->type == urdf::Joint::FIXED) {
			rbdl_joint = RigidBodyDynamics::Joint (RigidBodyDynamics::JointTypeFixed);
		} else if (urdf_joint->type == urdf::Joint::FLOATING) {
			// todo: what order of DoF should be used?
			rbdl_joint = RigidBodyDynamics::Joint (
		...
		RigidBodyDynamics::Math::Vector3d joint_rpy;
		RigidBodyDynamics::Math::Vector3d joint_translation;
		...
		RigidBodyDynamics::Math::SpatialTransform rbdl_joint_frame =
				RigidBodyDynamics::Math::Xrot (joint_rpy[0], RigidBodyDynamics::Math::Vector3d (1., 0., 0.))
				* RigidBodyDynamics::Math::Xrot (joint_rpy[1], RigidBodyDynamics::Math::Vector3d (0., 1., 0.))
				* RigidBodyDynamics::Math::Xrot (joint_rpy[2], RigidBodyDynamics::Math::Vector3d (0., 0., 1.))
				* RigidBodyDynamics::Math::Xtrans (RigidBodyDynamics::Math::Vector3d (
							joint_translation
							));

		// assemble the body
		RigidBodyDynamics::Math::Vector3d link_inertial_position;
		RigidBodyDynamics::Math::Vector3d link_inertial_rpy;
		RigidBodyDynamics::Math::Matrix3d link_inertial_inertia = RigidBodyDynamics::Math::Matrix3d::Zero();
		...
		if (urdf_child->inertial) {
			...
			if (link_inertial_rpy != RigidBodyDynamics::Math::Vector3d (0., 0., 0.)) {
		...
		RigidBodyDynamics::Body rbdl_body = RigidBodyDynamics::Body (link_inertial_mass, link_inertial_position, link_inertial_inertia);
		...
		if (urdf_joint->type == urdf::Joint::FLOATING) {
			RigidBodyDynamics::Math::Matrix3d zero_matrix = RigidBodyDynamics::Math::Matrix3d::Zero();
			RigidBodyDynamics::Body null_body (0., RigidBodyDynamics::Math::Vector3d::Zero(3), zero_matrix);
			RigidBodyDynamics::Joint joint_txtytz(RigidBodyDynamics::JointTypeTranslationXYZ);
			...
			RigidBodyDynamics::Joint joint_euler_zyx (RigidBodyDynamics::JointTypeEulerXYZ);
			rbdl_model->AppendBody (RigidBodyDynamics::Math::SpatialTransform(), joint_euler_zyx, rbdl_body, urdf_child->name);
		} else {
...
bool URDFReadFromString (const char* model_xml_string, RigidBodyDynamics::Model* model, bool floating_base, bool verbose) {
	assert (model);
	...
	cout << "URDFREADER: MODEL LOADED!!!!!!!!!!!!!!!!!!!!!" << endl;
	return true;
}

bool URDFReadFromFile (const char* filename, RigidBodyDynamics::Model* model, bool floating_base, bool verbose) {
